...
function handleServiceError(res, err) {
  ...
  logger.error(`管理生成记录接口异常：${err.message}`, { stack: err.stack });
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const result = await RecordService.listAdminRecords({
        page: req.query.page,
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const record = await RecordService.getAdminRecord(req.params.id);
